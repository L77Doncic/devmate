...
每个会话同时**只有一个写入实例**：同一会话 ID 的并发 append 属于误用（resume 应等旧实例退出，而不是与其并行写）。防御措施：文件适配器每次 append 从磁盘实际尾部的最后一个完整事件推导下一个 seq，并与实例缓存比对——不一致（另一实例续写过）即抛 SessionSeqConflictError，而不是静默复用陈旧 seq：重复 seq 行会被读端幂等去重静默丢弃，造成新事件无声消失。
