...
- **原生 Web UI**（[`src/ui/web`](src/ui/web)）——零框架 HTML/CSS/ES Modules、无构建步骤；双主题（浅色 GitHub / 深色 GitHub token 体系）、按工作区分组侧栏、13 条 `/` 命令、思考强度 pill（关闭/低/中/高）、上下文占用环、运行状态条、压缩披露小记。
...
| 方法                  | 路径                                                      | 用途                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`                 | `/api/stream?sessionId=…`                                 | SSE 帧流：`session-user`、`assistant-delta`、`reasoning`、`assistant-done`、`tool-start`、`tool-result`、`approval-request`、`usage`、`run-status`、`run-error`、`compaction`                                                                                                                                                                                                                                                                                                                                                                         |
| `POST`                | `/api/chat`                                               | 发消息 / 启动（或续跑）一轮 run；可选 `images: [{ref?,url?,width?,height?}]`（≤20 张；ref=sha256 内容寻址引用，或 legacy `data:image/…` dataURL，超限 413）；返回 `{sessionId}`（回显 `session-user` 帧携带同形 `images`）                                                                                                                                                                                                                                                                                                                                                                                  |
| `POST`                | `/api/attachments`                                        | 图片上传（内容寻址）：body `{sessionId, dataUrl}`（或 `{sessionId, data, mediaType}`）→ 200 `{ref: "sha256/…", width?, height?}`；≤20MiB/图、每条消息 ≤20 张、单会话累计 ≤200MiB（超限 413 带 error code）                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `GET`                 | `/api/attachments/<ref>`                                  | 原始图像字节读回（immutable 缓存；UI `img.src` 直指）——ref 缺失/非法 → 404                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `POST`                | `/api/approval`                                           | 应答待处理审批（`approve` / `deny` + 可选理由）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `POST`                | `/api/interrupt`                                          | 中断运行中的智能体（终态 `run-status` 仍经流到达）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `GET`/`POST`          | `/api/settings`                                           | 读返回 `{baseUrl, model, reasoning, apiKey?（掩码）, window?, maxInputTokens, maxOutputTokens, maxInputTokensDefault?, maxOutputTokensDefault?, maxInputTokensClamped?, maxOutputTokensClamped?, permission, methodFirst, reviewMode, windowDetail?, modelSanitized?, workspaceDir?, permissionConfirmedAt?}`（`model` 恒净化名——无 `[N]m/k` UI 尾标；上限恒回显，缺失回填缺省+Default 标记；`*Clamped` = 保存值被钳到供应商上限；`workspaceDir` = 该会话登记的工作区根；`windowDetail?` = 窗口三源判定细节）；写**必填** `{baseUrl, model, maxInputTokens, maxOutputTokens}` + 可选 `{apiKey?, reasoning?, windowTokens?, permission?, methodFirst?, reviewMode?}` —— 密钥只回掩码；上限缺失 → 400 `max-input-output-required` |
| `GET`/`POST`/`DELETE` | `/api/sessions`、`/api/sessions/:id`                      | 会话列表（每项含 `workspaceRoot`）、历史回放（≤ 500 帧）、新建、删除（活跃时 409）                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `GET`                 | `/api/workspaces`                                         | 已登记工作区根：`{roots: [...]}`（默认根为隐式首项）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `POST`                | `/api/workspaces`                                         | 登记工作区根——body `{path}`（绝对路径且为目录；canonical 归一；重复注册幂等）→ 200 `{roots: [...]}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `GET`                 | `/api/workspaces/browse?path=…`                           | 目录选择器只读列表（缺省 `os.homedir()`）——纯展示，不经过任何写入路径                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `DELETE`              | `/api/workspaces/:encodedRoot`                            | 注销工作区根（段内 `/` 以 `%2F` 转义）：默认根 → 400；未登记 → 404                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `GET`/`POST`/`DELETE` | `/api/skills` · `/api/skills/:id` · `/api/skills/install` | 技能清单（built-in + 用户安装，每项含 `origin`）、启用开关、用户技能安装——本地技能目录或 `raw.githubusercontent.com` URL；卸载（`DELETE /api/skills/:id`：仅 user 源可删；bundled → 404「内置技能不可移除」；未知 id → 404）。技能=目录（`SKILL.md`+文本资产，注入 ≤20k 字符）；URL 安装只落单文件 `SKILL.md`。                                                                                                                                                                                                                                                                                                                              |
