 ...
 *   三节初值（workflow 初值夹紧 1-4、skillsRecord 缺省 {}、mcpServers 缺省 []——
...
/**
 * maxParallel 初值夹紧 1-4（单一来源：shared/workflow 的 clampMaxParallel——三处副本
 * 已收敛到该模块；undefined → 2；floor 后夹紧）。只作用于**初值加载**；
 * 服务端已在 POST /api/workflow 校验越界 → 400（CLI 落盘透传不归一）。
 */
