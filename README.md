...
- **Subagent workflows** — `spawn_subagent` runs independent subtasks with a configurable parallel pool (`maxParallel` 1–4, default 2), enabled in Settings → Subagent; spawning with `skill:"code-review"` injects that skill's text (head-capped at 8000 code points) into the sub-agent's context (adoption of Claude Code subagent `skills` semantics — reviewer and main agent act on the same methodology text).
...
| `GET`/`POST`          | `/api/skills` · `/api/skills/:id` · `/api/skills/install` | Skill list (bundled + user-installed, each with `origin`), enable toggle, and user-skill install — local skill directory or `raw.githubusercontent.com` URL. A skill is a directory (`SKILL.md` + text assets, injected ≤ 20k chars); URL installs carry a single `SKILL.md` file.                                                                                                                                                                                                                                                                                                                                                                           |
