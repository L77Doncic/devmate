...
- **Request-side token limits (Settings)** — optional `输入上限 / 输出上限` (positive integers; blank = provider default). Output limit maps to `max_tokens` (`max_completion_tokens` for OpenAI/Kimi); the input limit is only sent to whitelisted providers (DashScope/Qwen `max_input_tokens` via `extra_body` — DeepSeek's API has no such parameter, documented as unimplemented).
...
| `GET`/`POST`          | `/api/settings`                                      | Read returns `{baseUrl, model, reasoning, apiKey? (masked), window?, maxInputTokens?, maxOutputTokens?}`; write accepts `{baseUrl, model, apiKey?, reasoning?, windowTokens?, maxInputTokens?, maxOutputTokens?}` — keys only ever echoed masked |
