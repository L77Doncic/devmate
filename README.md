...
| `GET`/`POST`          | `/api/settings`                                      | Read returns `{baseUrl, model, reasoning, apiKey? (masked), window?, maxInputTokens, maxOutputTokens, maxInputTokensDefault?, maxOutputTokensDefault?}` (`model` is always sanitized — no `[N]m/k` UI suffix; token limits are always present, unset values backfilled with defaults); write **requires** `{baseUrl, model, maxInputTokens, maxOutputTokens}` plus optional `{apiKey?, reasoning?, windowTokens?, permission?, methodFirst?, reviewMode?}` — keys only ever echoed masked; missing token limits → 400 `max-input-output-required` |
