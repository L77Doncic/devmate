...
- **Subagent workflows** — `spawn_subagent` runs independent subtasks with a configurable parallel pool (`maxParallel` 0–8, default 2; 0 = no cap), enabled in Settings → Subagent; spawning with `skill:"code-review"` injects that skill's text (head-capped at 8000 code points) into the sub-agent's context (adoption of Claude Code subagent `skills` semantics — reviewer and main agent act on the same methodology text). Sub-agents get a read-only tool surface (read_file / grep / glob / list_dir), bounded to ≤6 steps, with reports truncated at 4000 characters back into the parent; result content is redacted with the same single redactor as the main loop, and a review sentinel can spawn an independent code-review subagent before natural completion.
...
npm test               # vitest run (134 test files, 1964 cases — 1963 passed, 1 skipped; full E2E suite runs on POSIX in CI — the Windows job gates lint + typecheck + build, and the win32 shell path (Git Bash / PowerShell probe) is exercised there by launch smoke, not by the POSIX-authored E2E semantics)
npm run test:watch
